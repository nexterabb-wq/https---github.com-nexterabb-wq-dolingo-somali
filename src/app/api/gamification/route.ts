  ...
  try {
    ...
    return NextResponse.json({
      ...
      currentLevel: currentLevel
        ? { level: currentLevel.level, title: currentLevel.title, icon: currentLevel.icon }
        : null,
      nextLevel: nextLevel
        ? { level: nextLevel.level, title: nextLevel.title, xpRequired: nextLevel.xpRequired }
        : null,
      levelProgress: Math.min(levelProgress, 1),
      levels: levels.map((l) => ({ level: l.level, title: l.title, xpRequired: l.xpRequired, icon: l.icon })),
